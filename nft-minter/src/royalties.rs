...
pub trait RoyaltiesModule: crate::admin_whitelist::AdminWhitelistModule + pause::PauseModule {
    ...
    fn claim_royalties(&self) -> MoaxValuePaymentsVecPair<Self::Api> {
    ...
    fn claim_mint_payments(&self) -> MoaxValuePaymentsVecPair<Self::Api> {
        self.require_not_paused();
    ...
    fn claim_common(
        ...
        mapper: &mut MapMapper<MoaxOrDctTokenIdentifier, BigUint>,
    ) -> MoaxValuePaymentsVecPair<Self::Api> {
        let caller = self.blockchain().get_caller();
        require!(caller == claim_allowed_address, "Claim not allowed");

        let mut moax_value = BigUint::zero();
        let mut other_payments = ManagedVec::new();
        for (token, amount) in mapper.iter() {
            if token.is_moax() {
                moax_value = amount;
            } else {
                other_payments.push(DctTokenPayment::new(token.unwrap_dct(), 0, amount));
            }
        }
        ...
        if moax_value > 0 {
            self.send().direct_moax(&caller, &moax_value);
        }
        if !other_payments.is_empty() {
            self.send().direct_multi(&caller, &other_payments);
        }

        (moax_value, other_payments).into()
    }

    fn add_mint_payment(&self, token: MoaxOrDctTokenIdentifier, amount: BigUint) {
        let mut mapper = self.accumulated_mint_payments();
        self.add_common(&mut mapper, token, amount);
    }

    fn add_royalties(&self, token: MoaxOrDctTokenIdentifier, amount: BigUint) {
        let mut mapper = self.accumulated_royalties();
        self.add_common(&mut mapper, token, amount);
    }

    fn add_royalties_multiple(&self, payments: &ManagedVec<DctTokenPayment<Self::Api>>) {
        let mut mapper = self.accumulated_royalties();
        for p in payments {
            self.add_common(
                &mut mapper,
    ...
    fn accumulated_royalties(&self) -> MapMapper<MoaxOrDctTokenIdentifier, BigUint>;

    #[view(getAccumulatedMintPayments)]
    #[storage_mapper("accumulatedMintPayments")]
    fn accumulated_mint_payments(&self) -> MapMapper<MoaxOrDctTokenIdentifier, BigUint>;
}
