...
use dharitri_sc::{
    ...
    types::{Address, DctLocalRole, ManagedVec, MultiValueEncoded},
};
...
macro_rules! managed_token_id {
    ($bytes:expr) => {{
        if $bytes
            == dharitri_sc::types::MoaxOrDctTokenIdentifier::<DebugApi>::MOAX_REPRESENTATION
        {
            dharitri_sc::types::MoaxOrDctTokenIdentifier::moax()
        } else {
            let ___token_id___ = dharitri_sc::types::TokenIdentifier::from_dct_bytes($bytes);
            dharitri_sc::types::MoaxOrDctTokenIdentifier::dct(___token_id___)
        }
    }};
}
    ...
    NftMinterObjBuilder: 'static + Copy + Fn() -> nft_minter::ContractObj<DebugApi>,
{
    pub fn new(builder: NftMinterObjBuilder) -> Self {
        ...
        let owner_address = b_mock.create_user_account(&rust_biguint!(OWNER_MOAX_BALANCE));
        let first_user_address = b_mock.create_user_account(&rust_biguint!(USER_MOAX_BALANCE));
        let second_user_address = b_mock.create_user_account(&rust_biguint!(USER_MOAX_BALANCE));
        let nm_wrapper =
            b_mock.create_sc_account(&rust_zero, Some(&owner_address), builder, "nft minter path");

        // init DCT System SC mock
        b_mock.create_sc_account_fixed_address(
            &Address::from(DCT_SYSTEM_SC_ADDRESS_ARRAY),
            &rust_zero,
            None,
            dct_system_sc_mock::contract_obj,
            "DCT system SC mock path",
        );
    ...
    NftMinterObjBuilder: 'static + Copy + Fn() -> nft_minter::ContractObj<DebugApi>,
{
    ...
    pub fn call_buy_random_nft(
        ...
        if payment_token == MOAX_TOKEN_ID {
            self.b_mock.execute_tx(
                buyer_address,
                &self.nm_wrapper,
