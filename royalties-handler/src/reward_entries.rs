...
pub struct RewardEntry<M: ManagedTypeApi> {
    ...
    pub dct_payments: PaymentsVec<M>,
}
    ...
    crate::common_storage::CommonStorageModule + crate::token_balance::TokenBalanceModule
{
    ...
    fn create_new_reward_entry(&self) {
        ...
        let mut dct_payments = PaymentsVec::new();
        for token_id in self.known_tokens().iter() {
            ...
            if token_id.is_moax() {
                moax_amount = amount_per_holder;
            } else {
                dct_payments.push(DctTokenPayment::new(
                    token_id.unwrap_dct(),
                    0,
                    amount_per_holder,
                ));
        ...
        if moax_amount > 0 || !dct_payments.is_empty() {
