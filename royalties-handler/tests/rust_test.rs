...
fn claim_payments_test() {
    ...
    // check balance
    rh_setup.b_mock.check_dct_balance(
        rh_setup.rh_wrapper.address_ref(),
        ROYALTIES_TOKEN_ID,
        &rust_biguint!(ROYALTIES_TOKEN_BALANCE),
    );
    rh_setup.b_mock.check_moax_balance(
        rh_setup.rh_wrapper.address_ref(),
    ...
    rh_setup
        ...
        .execute_query(&rh_setup.rh_wrapper, |sc| {
            ...
            assert!(sc
                .known_tokens()
                .contains(&managed_token_id!(MOAX_TOKEN_ID)));
            ...
            assert_eq!(
                sc.balance_for_token(&managed_token_id!(MOAX_TOKEN_ID))
                    .get(),
        ...
        .execute_query(&rh_setup.rh_wrapper, |sc| {
            ...
            assert_eq!(
                ...
                RewardEntry::<DebugApi> {
                    moax_amount: managed_biguint!(MINT_PAYMENTS_BALANCE / 3),
                    dct_payments: ManagedVec::from_single_item(DctTokenPayment::<DebugApi>::new(
                        managed_token_id!(ROYALTIES_TOKEN_ID).unwrap_dct(),
                        0,
...
fn claim_rewards_test() {
    ...
    rh_setup
        .b_mock
        .check_moax_balance(&first_sh, &rust_biguint!(MINT_PAYMENTS_BALANCE / 3));
    ...
    rh_setup
        .b_mock
        .check_moax_balance(&second_sh, &rust_biguint!(MINT_PAYMENTS_BALANCE / 3));
    ...
    rh_setup
        .b_mock
        .check_moax_balance(&third_sh, &rust_biguint!(MINT_PAYMENTS_BALANCE / 3));
